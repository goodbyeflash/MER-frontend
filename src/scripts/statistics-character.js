        ...
        document.getElementsByClassName('btn btn-excel')[0].onclick = () => {
          api(
            ...
            (res) => {
              ...
              a.download = `통계관리(캐릭터별) ${sDate}-${eDate}.xlsx`;
              a.click();
            }
          );
        };
      ...
      (res) => {
        if (res.msg == 'OK') {
          ...
          items.forEach((item) => {
            ...
            item.data_key_value_list.forEach((list,index)=> {
              const value = parseInt(list.v);
              // 선택 한 사람 수 값 입력
              chartData[contentIndex-1].data[value] = chartData[contentIndex-1].data[value]+1;              
              totalCount++;
            });
          });          
          ...
          console.error('[API] => 캐릭터 별 데이터를 불러올 수 없습니다.');
        }
      }
      );     
