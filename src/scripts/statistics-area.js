        ...
        document.getElementsByClassName('btn btn-excel')[0].onclick = () => {
          ...
          api(
            ...
            (res) => {
              ...
              a.download = `통계관리(지역별-${address}) ${sDate}-${eDate}.xlsx`;
              a.click();
            }
          );
        };
...
function onLoadData() {
  const addressSelect1 = document.getElementById('sel1');
  const addressSelect2 = document.getElementById('sel2');
...
async function onLoadChart() {
  ...
  for await (const key of charNums) {
    ...
    statisticsApi('post', 'statistics/address', sendData, key, (res) => {
      if (res.msg == 'OK') {
        ...
        const chartData = [
          ...
          {
            //데이터
            label: '자전거레일과 짐경사로가 있는 계단용', //차트 제목
            data: [
              0,
              0,
              0,
        ...
        console.error('[API] => 지역 별 데이터를 불러올 수 없습니다.');
      }
    });
  }
}
...
function onLoadKoreaAddress() {
  const addressEl = document.getElementById('sel1');
...
function onChangeKoreaAddress() {
  const cityEl = document.getElementById('sel2');
  const key = document.getElementById('sel1').value;
  cityEl.innerHTML = '';
